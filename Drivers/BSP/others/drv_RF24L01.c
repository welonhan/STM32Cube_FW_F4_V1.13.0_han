...
{
	uint8_t i;
	uint8_t buf[5]={ 0xa4,0xa5,0xcc,0xdd,0xf8 };
...
/**
  ...
  *			0xFF:其他原因
  */ 
void NRF24L01_TxPacket( uint8_t *txbuf, uint8_t Length )
{
	//uint8_t Status = 0;
...
/**
  ...
  * @retval:
  *			MAX_TX：达到最大重发次数
  *			TX_OK|RX_OK：发送完成，接收到正确的ACK
  *			0: 错误
  */ 
uint8_t NRF24L01_IRQ( uint8_t *rxbuf, uint8_t ack_num )
{	
	uint8_t Status = 0;
	...
	if( Status & (TX_OK|RX_OK) )										//接收到ADC发送完成
	{
		NRF24L01_Read_Buf( RD_RX_PLOAD,rxbuf,ack_num );		//接收到数据 
		NRF24L01_Write_Reg( FLUSH_RX,0xff );							//清除RX FIFO
		return (TX_OK|RX_OK);															//接收到ACK
	}
	return 0;	
}
...
void NRF24L01_Gpio_Init( void )
{
  ...
  RF24L01_CE_GPIO_CLK_ENABLE();
	RF24L01_IRQ_GPIO_CLK_ENABLE();
  RF24L01_CS_GPIO_CLK_ENABLE();
	
	/* Configure CE pin as output pullup */
	GPIO_InitStruct.Mode=GPIO_MODE_OUTPUT_PP;
  //GPIO_InitStruct.Pull = GPIO_PULLUP;
  GPIO_InitStruct.Speed = GPIO_SPEED_FAST;
  GPIO_InitStruct.Pin = RF24L01_CE_GPIO_PIN;
  HAL_GPIO_Init(RF24L01_CE_GPIO_PORT, &GPIO_InitStruct);
	
	/* Configure CS pin as output pullup */
  GPIO_InitStruct.Pin = RF24L01_CS_GPIO_PIN;
	HAL_GPIO_Init(RF24L01_CS_GPIO_PORT, &GPIO_InitStruct);
	
	/* Configure IRQ pin as input with External interrupt */
	//GPIO_InitStruct.Mode=GPIO_MODE_INPUT;
	GPIO_InitStruct.Pull = GPIO_NOPULL;
  GPIO_InitStruct.Pin = RF24L01_IRQ_GPIO_PIN;
	GPIO_InitStruct.Mode = GPIO_MODE_IT_FALLING;
	HAL_GPIO_Init(RF24L01_IRQ_GPIO_PORT, &GPIO_InitStruct);
	
  /* Enable and set IRQ EXTI Interrupt to the highest priority */
...
void NRF24L01_Init( void )
{
    ...
    NRF24L01_Write_Reg( FEATRUE, 0x06 );			//使能动态数据长度，使能ACK
    ...
    NRF24L01_Write_Reg( SETUP_RETR, ARD_2000US |
                        ( REPEAT_CNT & 0x0F ) );         					//重复等待时间 500us, 重复传输15次
    NRF24L01_Write_Reg( RF_CH, 60 );             									//初始化通道
    NRF24L01_Write_Reg( RF_SETUP, 0x0E );													//2Mbps 0dbm
