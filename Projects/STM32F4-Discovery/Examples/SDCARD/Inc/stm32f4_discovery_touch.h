...
#define FT_SCL_Pin   GPIO_Pin_5
#define FT_SDA_Pin   GPIO_Pin_12   
#define FT_INT_Pin   GPIO_Pin_4   //????
			
#define I2C1_SCL_OUT(x) 	\
						if (x)	\
						GPIO_SetBits(GPIOC,FT_SCL_Pin);	\
						else		\
						GPIO_ResetBits(GPIOC,FT_SCL_Pin);
						
#define I2C1_SDA_OUT(x)	\
						if (x)	\
						GPIO_SetBits(GPIOB,FT_SDA_Pin);	\
						else		\
						GPIO_ResetBits(GPIOB,FT_SDA_Pin);


#define	I2C1_SDA_IN()   GPIO_ReadInputDataBit(GPIOB,FT_SDA_Pin)
