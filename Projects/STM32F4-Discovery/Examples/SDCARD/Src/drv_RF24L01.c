...
/**
  ...
  *			RX_OK: 接收原因
  */ 
uint8_t NRF24L01_IRQ( uint8_t *rxbuf )
{	
	uint8_t Status = 0, count;
		
	Status = NRF24L01_Read_Reg(STATUS);						//读状态寄存器
	count= NRF24L01_Read_Reg(OBSERVE_TX);					//读状态寄存器
	...
	if( Status & (TX_OK|RX_OK) )	//发送完成
	{
		NRF24L01_Read_Buf( RD_RX_PLOAD,rxbuf,32 );	//接收到数据 
